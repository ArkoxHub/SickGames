 ...
 *
 * @author Adri
 * @version 1.0 Controlador encarregat d'atendre a les peticions que tenen a
 * veure amb l'administrador i que correspon a totes les accions que un
 * administrador de la web pot fer, tal com accedir al menú d'administradors,
 * CRUD de Videojocs, Codis, Usuaris...
 */
@Controller
public class adminController {
     ...
     * Gràcies als arxius de configuració fet a security-context.xml, aquest
     * mètode serà inaccesible fins que la persona que intenta accedir no validi
     * els seus credencials a la pàgina interceptada que correspon amb el mètode
     * adminLogin d'aquesta mateixa classe
     *
     * @param request
     * @param response
     * @return admin.jsp
    ...
    public ModelAndView adminMainPage(HttpServletRequest request, HttpServletResponse response)
        ...
        model.getModelMap().addAttribute("admin", username);
     ...
     * www.sickgames.com/admin/**. Retorna un model que demanarà els credencials
     * d'administrador per a poder accedir a la pàgina d'admin.
     *
     * @param request
     * @param response
    ...
    /**
     * Aquest mètode és l'encarregat d'interceptar una petició errònea d'un
     * intent de login d'administrador incorrecte. Afegeix l'atribut error =
     * true a la resposta.
     *
     * @param model
     * @return /spring/login per a tornar a intentar validar credencials
     * d'admin, aquest cop li mostrarà un missatge d'error
     */
    ...
    /**
     * Mètode que rep les dades del formulari per a crear un videojoc omplert
     * per un administrador mitjançant POST.
     *
     ...
     * serà l'encarregat de rebre aquesta petició. Obté el joc en qüestió a
     * través del seu ID passat per paràmetre i l'afegeix a l'atribut del model
     * que veurà en la vista formulari retornat.
     *
     ...
     *
     * Realitza el tractament per guardar la imatge de la caràtula en el
     * servidor.
     *
    ...
    /**
     * Obté tot el llistat de jocs disponibles a la pàgina web de la base de
     * dades i l'adjunta al model. Se li pot passar per URL si vol procedir a
     * modificar o eliminar aquests jocs al fer click en un d'ells.
     *
    ...
    /**
     * Quan l'administrador selecciona un videojoc per a ser eliminat de la
     * llista, aquest mètode s'encarrega de procedir amb aquesta petició.
     *
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/admin/deleteGame", method = RequestMethod.GET)
    public ModelAndView deleteGame(@RequestParam("id") int codi,
        ...
        model.getModelMap().addAttribute("videojocs", videojocService.getAllGames());
        model.getModelMap().addAttribute("option", "Delete");
        return model;
    }

    /**
     * Si l'administrador vol crear un joc, aquest mètode és l'encarregat de
     * crear un objecte codi nou que l'adjuntarà al model retornat per tal de
     * que procedeixi amb la escriptura de les seves dades.
     *
     * @param request
     * @param response
    ...
    /**
     * Rep el formulari de creació d'un codi de joc nou omplert per
     * l'administrador mitjançant el mètode POST.
     *
     * Es calcula el preu del codi mitjançant el PVP del Videojoc i el %
     * d'Oferta aplicada al codi en qüestió.
     *
    ...
    /**
     * Rep la petició quan un administrador vol modificar o eliminar codis
     * retornant la vista codeList.jsp.
     *
     * @param option valor que pot variar entre "modify" o "delete"
     * @param request
     * @param response
    ...
    /**
     * Rep les peticiones fetes a la vista codeList.jsp, en la que
     * l'administrador ha especificat quins codis de videojoc vol modificar o
     * eliminar a través del codi del videojoc mitjançant mètode POST.
     *
     * Consulta a la Base de Dades tots els codis que tenim referents al
     * Videojoc especificat. Si tenim resultat, s'afegeixen a l'atribut del
     * model. Si no, es mostra un error.
     *
     * @param option
     * @param codi
     * @param request
    ...
    /**
     * Si l'administrador des codeList.jsp clica sobre un codi a modificar,
     * aquest mètode obté de la base de dades l'objecte codi en qüestió i
     * l'adjunta al model.
     *
     * @param codi
     * @param request
     * @param response
     * @return Formulari updateCodeForm.jsp per tal de modificar les dades del
     * codi
     * @throws ServletException
     * @throws IOException
     */
    ...
    /**
     * Rep el codi modificat per part de l'admin en el formulari mitjançant
     * POST.
     *
     * Es calcula el preu del codi mitjançant el PVP del Videojoc i el %
     * d'Oferta aplicada al codi en qüestió.
     ...
     * @param codi_Joc Codi del Videojoc que fa referència el codi que es vol
     * eliminar
     * @param request
     * @param response
     * @return codeList.jsp
